...
Once the script is run in local environment following the [instructions](../../AppCreationScripts/README.md), the `Microsoft Entra` fields are updated in **src/appsettings.json** from service app registration and `msalConfig.auth` fields are updated in **MinimalClient/src/authConfig.js** from client app registration in the cloned repository.
