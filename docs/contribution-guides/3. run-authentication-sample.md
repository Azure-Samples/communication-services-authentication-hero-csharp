...
3. Make a GET request to `http://localhost:3000/api/token` with the `access_token` generated at **step 2** of **Generate an Azure AD Token manually** guide as a Authorization Bearer header. Verify you get a response with a successful status code (i.e. 200).
